...
INSERT INTO developers (username, nickname, member_uid, email, password, role, enabled, email_verified, deleted, login_fail_count, created_at, updated_at)
SELECT 'admin', '管理员', 'admin-001', 'admin@zutils.com', '$2a$10$3gB1/ITiblcFP./WpdnXReko/DLGdw7nrm155U2Cuv3c4p7kXoU7e', 'ADMIN', true, true, false, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM developers WHERE username = 'admin');

INSERT INTO developers (username, nickname, member_uid, email, password, role, enabled, email_verified, deleted, login_fail_count, created_at, updated_at)
SELECT 'zutils-team', 'ZUtils Team', 'zutils-team-001', 'team@zutils.com', '$2a$10$3gB1/ITiblcFP./WpdnXReko/DLGdw7nrm155U2Cuv3c4p7kXoU7e', 'DEVELOPER', true, true, false, 0, NOW(), NOW()
...
INSERT INTO plugins (id, function_name, description, icon, category, author, min_app_version, downloads, rating, developer_id, created_at, updated_at)
SELECT 'plugin_toastcontent', 'toastContent', '安卓Toast函数', '', 'UTILITY', 'ZUtils Team', '1.0.0', 0, 0.0, (SELECT id FROM developers WHERE username = 'zutils-team'), NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM plugins WHERE id = 'plugin_toastcontent');
...
INSERT INTO plugin_versions (plugin_id, version, dex_url, dex_size, class_name, parameters, required_permissions, dependencies, status, published_at)
SELECT 'plugin_toastcontent', '1.0.0', 'http://localhost:8080/api/v1/files/plugin_toastContent_v1.0.0.dex', 3880, 'com.zutils.generated.toastContentFunction', '[{"name":"content","description":"提示内容","type":"STRING","required":true}]', '[]', '[]', 'APPROVED', NOW()
WHERE NOT EXISTS (SELECT 1 FROM plugin_versions WHERE plugin_id = 'plugin_toastcontent' AND version = '1.0.0');
